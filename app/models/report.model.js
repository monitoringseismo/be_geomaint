...
class Report {
    ...
    async insert(data){
        ...
        const result = await db.collection('report').insertOne(data);
    ...
    async list(filter, sort, limit){
        ...
        const result = await db.collection('report').find(filter).sort(sort).limit(limit);
    ...
    async show(filter, projection = {}){
        const db = await this.getInstance();
        filter.deleted_at = {$exists:false}
        const result = await db.collection('report').findOne(filter, projection)
        return result;
    }
    ...
    async update(id, upd){
        ...
        await db.collection('report').updateOne(id,upd);
    ...
    async addLastActivityDate(nsim){
        ...
        await db.collection('report').updateMany(id,upd);
    }

    async delete(id){
        const db = await this.getInstance();
        var update = {$set:{deleted_at:new Date()}};
        const result = await db.collection('report').updateOne({_id:new ObjectId(id), deleted_at:{$exists : false}},update);
